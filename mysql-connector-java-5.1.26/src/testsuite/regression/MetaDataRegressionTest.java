...
public class MetaDataRegressionTest extends BaseTestCase {
	...
	private void testBug65871_testCatalogs(Connection conn1) throws Exception {
		testBug65871_testCatalog(
				"db1testbug65871",
				StringUtils.quoteIdentifier("db1testbug65871", ((ConnectionProperties)conn1).getPedantic()),
				conn1);

		testBug65871_testCatalog(
				"db2testbug65871",
				StringUtils.quoteIdentifier("db2testbug65871", "\"", ((ConnectionProperties)conn1).getPedantic()),
				conn1);

		try {
			testBug65871_testCatalog(
					"db3testbug65871",
					StringUtils.quoteIdentifier("db3testbug65871", "\"", ((ConnectionProperties)conn1).getPedantic()),
					conn1);
			assertTrue("Driver should mistake about db3testbug65871 in non-pedantic mode",((ConnectionProperties)conn1).getPedantic());
		} catch (Exception e) {
			System.out.println(e.getMessage());
			assertFalse("Driver should not mistake about db3testbug65871 in pedantic mode",((ConnectionProperties)conn1).getPedantic());
		}
	}
	...
	private void testBug65871_testCatalog(
		...
		try {
			...
			testBug65871_testTable(
					...
					"table1testbug65871",
					StringUtils.quoteIdentifier("table1testbug65871", ((ConnectionProperties)conn1).getPedantic()),
					conn1, st1);

			testBug65871_testTable(
					unquotedDbName, quotedDbName,
					"table2testbug65871",
					StringUtils.quoteIdentifier("table2testbug65871", "\"", ((ConnectionProperties)conn1).getPedantic()),
					conn1, st1);
			...
			try {
				testBug65871_testTable(
						unquotedDbName, quotedDbName,
						"table4testbug65871",
						StringUtils.quoteIdentifier("table4testbug65871", "\"", ((ConnectionProperties)conn1).getPedantic()),
						conn1, st1);
				assertTrue("Driver should mistake about table4testbug65871 in non-pedantic mode",((ConnectionProperties)conn1).getPedantic());
			} catch (Exception e) {
				System.out.println(e.getMessage());
				assertFalse("Driver should not mistake about table4testbug65871 in pedantic mode",((ConnectionProperties)conn1).getPedantic());
			}
	...
	private void testBug65871_testTable(
		...
		try {
			...
			try {
				boolean found = false;
				this.rs = conn1.getMetaData().getColumns(unquotedDbName, null, unquotedTableName, "BEST");
				while (this.rs.next()) {
					if ("BEST".equals(this.rs.getString("COLUMN_NAME"))) {
						found = true;
					}
				}
				if (!found) {
			...
			try {
				...
				if (!this.rs.next() || !"BEST".equals(this.rs.getString("COLUMN_NAME"))) {
